...
const isObjectLike = obj => !isOmit(obj) && !nonObjectLikeTypes.includes(obj.constructor?.name)
...
const serializeObjectLike = (obj, ignores, prefix) => {
    if (isObjectLike(obj)) {
        const origin = obj?.__origin__ ?? obj?.getOrigin?.() ?? {}
        const result = {}
...
export const deserialize = (obj, prefix = '') => {
    if (isObjectLike(obj)) {
        const result = {}
        Object.defineProperty(result, '__origin__', { value: obj })
